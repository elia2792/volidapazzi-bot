...
class MonetizationEngine:
    ...
    def generate_flight_search_url(self, departure: Optional[str], destination: Optional[str], fallback_url: str) -> str:
        ...
        if departure and destination:
            dep_clean = urllib.parse.quote(departure.strip())
            dest_clean = urllib.parse.quote(destination.strip())
            # Aviasales / Travelpayouts deep flight search URL
        ...
        # Cloak original URL with Travelpayouts tracking if applicable
    ...
    def generate_hotel_search_url(self, destination: Optional[str]) -> str:
        """
        Generates an affiliate hotel search URL.
        If booking_aid is customized, uses direct Booking.com AID,
        otherwise monetizes through Travelpayouts Booking campaign!
        """
        dest_query = destination if destination else "Offerte Hotel"
        encoded = urllib.parse.quote(dest_query)
        ...
        # Use Travelpayouts Booking.com campaign redirect
    ...
    def generate_activities_url(self, destination: Optional[str]) -> str:
        """
        Generates a Civitatis / GetYourGuide tour and activity affiliate URL.
        """
        ...
        # Fallback to Travelpayouts Tiqets / GetYourGuide campaign
        return f"https://tp.media/r?marker={self.tp_marker}&p=648&u={urllib.parse.quote(f'https://www.tiqets.com/it/search/?q={encoded}')}"

    def generate_esim_url(self) -> str:
        """
        Generates a Travelpayouts Yesim affiliate link.
        """
        return f"https://tp.media/r?marker={self.tp_marker}&p=5998&u=https%3A%2F%2Fyesim.tech"

    def generate_amazon_travel_gear_url(self, query: str = "zaino bagaglio a mano 40x20x25 ryanair") -> str:
        """
        Generates an Amazon affiliate link for travel accessories.
        """
    ...
    def build_guide_keyboard(self, destination: Optional[str] = None, amazon_query: Optional[str] = None) -> InlineKeyboardMarkup:
        """
        Builds inline buttons for travel guides (hotels, tours, flight search, travel gear).
        """
        buttons: List[List[InlineKeyboardButton]] = []
